...
<main class="main">
  ...
  <header class="top-header">
    ...
    <div class="top-header-menu">
      <a href="/_swagger" target="_blank" class="menu-btn">
        {{ t('header.testApi') }}
      </a>
      <NuxtLink to="/tasks" class="menu-btn">
        {{ t('header.tasksBoard') }}
      </NuxtLink>
      <a href="http://localhost:3000/h5/promo" target="_blank" class="menu-btn">
        {{ t('header.h5Promo') }}
      </a>
      <a href="http://admin.yourdomain.localhost:3000/admin" target="_blank" class="menu-btn btn-primary">
        {{ t('header.adminPortal') }}
      </a>
      <button @click="toggleLocale" class="menu-btn" style="padding:4px 8px;font-size:11px;">
        🌐 {{ localeLabel === '中文' ? 'EN' : '中' }}
      </button>
    </div>
  </header>
...
│   │   └── auth.ts                   <span class="cm"># useUser：客户端会话状态管理</span>
│   └── pages/
...
└── nuxt.config.ts                    <span class="cm"># 核心配置（路由规则·渲染模式·缓存·图片）</span></pre>
  ...
  <span class="kw">const</span> ROOT_DOMAIN = <span class="str">'yourdomain.com'</span>
