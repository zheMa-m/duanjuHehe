 ...
 * 管理后台主题状态管理
 *
 * 单一主题：classic-dark（经典暗色）
 *
 * CSS 自定义属性（CSS Variables）驱动，浏览器原生响应，零闪烁。
 */

export type AdminTheme = 'classic-dark'
export type ResolvedTheme = 'classic-dark'

const THEME_KEY = 'admin-theme'
const VALID_THEMES: AdminTheme[] = ['classic-dark']
...
function getStoredTheme(): AdminTheme {
  if (typeof localStorage === 'undefined') return 'classic-dark'
...
const resolvedTheme = ref<ResolvedTheme>('classic-dark')
...
export function useAdminTheme() {
  ...
  const colorScheme = computed(() => 'dark' as const)
