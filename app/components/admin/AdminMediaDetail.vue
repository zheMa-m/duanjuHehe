...
function confirmDelete() {
  if (!confirm(`确定要将「${props.file.name}」移入回收站吗？`)) return
...
<template>
  <div class="fixed top-0 right-0 bottom-0 w-[380px] bg-[#08080f]/95 backdrop-blur-2xl border-l border-white/[0.06] shadow-[-10px_0_30px_rgba(0,0,0,0.6)] z-40 flex flex-col overflow-hidden">
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-white/[0.06] flex-shrink-0">
      ...
      <button
        @click="$emit('close')"
        class="w-7 h-7 rounded-lg bg-white/[0.05] hover:bg-white/[0.10] flex items-center justify-center text-white/50 hover:text-white transition-all cursor-pointer text-xs"
      >✕</button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10">
      ...
      <!-- ── 基本信息 ───────────────────────────────────────── -->
      <div class="px-5 py-4 space-y-4 border-b border-white/[0.04]">
      ...
      <!-- ── EXIF 元数据 ───────────────────────────────────────── -->
      <div v-if="file.exif && Object.keys(file.exif).length > 0" class="px-5 py-4 space-y-4">
    ...
    <!-- ── 底部操作 ─────────────────────────────────────────── -->
    <div class="flex-shrink-0 px-5 py-4 border-t border-white/[0.06] flex gap-3">
      <button
        @click="confirmDelete"
